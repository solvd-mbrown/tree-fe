...
type AddSiblingsFormProps = {
	...
	setNewSiblingDetails?: (details: string) => void;
	newSibling?: boolean;
	setLoading: (load: boolean) => void;
	keyboardDismiss: () => void;
};

const AddSiblingsForm: FC<AddSiblingsFormProps> = ({
	firstName,
	lastName,
	birthdate,
	dateOfDeath,
	...
	const [deceased, setDeceased] = useState<string>(
		newSibling
			? ''
			: questionnaireSettings?.step4_AddSiblings?.formVariants &&
			  questionnaireSettings?.step4_AddSiblings?.formVariants[`sibling${siblingNumber}`]
			? questionnaireSettings?.step4_AddSiblings?.formVariants[`sibling${siblingNumber}`]?.deceased
			: ''
	);
	const [details, setDetails] = useState<string>(
		newSibling
			...
			: questionnaireSettings?.step4_AddSiblings?.formVariants &&
			  questionnaireSettings?.step4_AddSiblings?.formVariants[`sibling${siblingNumber}`]
			? questionnaireSettings?.step4_AddSiblings?.formVariants[`sibling${siblingNumber}`]?.details
			: ''
	);
	...
	const isDisabled = questionnaireSettings?.step4_AddSiblings?.formVariants
		? !!questionnaireSettings?.step4_AddSiblings?.formVariants[`sibling${siblingNumber}`]?.details
		: false;
	...
	const onSubmit = async (values: InitialValues) => {
		...
		if (authUser.data?.siblings && authUser.data?.siblings[siblingNumber - 1]) {
			response = await dispatch(
				updateFamilyMemberAndAuthUserAsync({
					authUserId: authUser.data?.id,
					userId: authUser.data?.siblings[siblingNumber - 1]?.identity,
					userData: {
				...
				<>
					<Box alignItems="center" justifyContent="center" mt={15} mb={5} width={'100%'}>
						<Box width={'90%'} position={'relative'}>
							<Text my={1}>
								{siblingNumber > 1 ? 'Add another siblings?' : 'Do you have siblings?'}
							</Text>
							<Radio.Group
								name="details"
								colorScheme="amber"
								...
								onChange={value => {
									props.handleReset();
									if (newSibling && setNewSiblingDetails) {
										setNewSiblingDetails(value);
									}
									setDetails(value);
									setDeceased('no');
							...
							{details === 'yes' && (
								<>
									<Text textAlign={'center'}>Sibling {siblingNumber}</Text>
...
export default AddSiblingsForm;
