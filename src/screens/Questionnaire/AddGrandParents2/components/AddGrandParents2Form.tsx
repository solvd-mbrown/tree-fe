...
type AddGrandParents2FormProps = {
	parent1?: boolean;
	parent2?: boolean;
...
const AddGrandParents2Form: FC<AddGrandParents2FormProps> = ({
	parent1,
	parent2,
	detailsParent1,
	detailsParent2,
	...
	const isDisabled = !!(
		...
		(parent2 && questionnaireSettings?.step6_AddParent2GrandParents?.formVariants?.parent2?.details)
	);

	const [deceased, setDeceased] = useState<string>(
		parent1
			? questionnaireSettings?.step6_AddParent2GrandParents?.formVariants?.parent1?.deceased
			: questionnaireSettings?.step6_AddParent2GrandParents?.formVariants?.parent2?.deceased
	);
	...
	const getInitialValues = (): InitialValues => {
		...
		if (user?.data?.parents?.length > 0) {
			InitialValues.firstName = parent1
				? user?.data?.parents[0]?.properties?.firstName
				: user?.data?.parents[1]?.properties?.firstName;
			InitialValues.lastName = parent1
				? user?.data?.parents[0]?.properties?.lastName
				: user?.data?.parents[1]?.properties?.lastName;
			InitialValues.birthdate = parent1
				? user?.data?.parents[0]?.properties?.birthdate
				: user?.data?.parents[1]?.properties?.birthdate;
			InitialValues.gender = parent1
				? user?.data?.parents[0]?.properties?.gender
				: user?.data?.parents[1]?.properties?.gender;
			InitialValues.dateOfDeath = parent1
				? user?.data?.parents[0]?.properties?.dateOfDeath
				: user?.data?.parents[1]?.properties?.dateOfDeath;
			InitialValues.anniversaryDate = parent1
				? user?.data?.parents[0]?.properties?.anniversaryDate
				: user?.data?.parents[1]?.properties?.anniversaryDate;
		}
	...
	const onSubmit = async (values: InitialValues) => {
		...
		if (
			user.data?.parents &&
			((parent1 && user?.data?.parents[0]) || (parent2 && user?.data?.parents[1]))
		) {
			await dispatch(
				updateFamilyMemberAndAuthUserAsync({
					grandParent: true,
					authUserId: authUser.data?.id,
					userId: parent1 ? user?.data?.parents[0]?.identity : user?.data?.parents[1]?.identity,
					childId: user?.data?.id,
					userData: {
					...
					setting: {
						...
						step6_AddParent2GrandParents: {
							...
							isGrandParent1Added: parent1
								? true
								: questionnaireSettings?.step6_AddParent2GrandParents?.isGrandParent1Added,
							isGrandParent2Added: parent2
								? true
								: questionnaireSettings?.step6_AddParent2GrandParents?.isGrandParent2Added,
							formVariants: {
								parent1: {
									details: parent1
										? detailsParent1
										: questionnaireSettings?.step6_AddParent2GrandParents?.formVariants?.parent1
												?.details,
									deceased: deceased
										? deceased
										: questionnaireSettings?.step6_AddParent2GrandParents?.formVariants?.parent1
								...
								parent2: {
									details: parent2
										? detailsParent2
										: questionnaireSettings?.step6_AddParent2GrandParents?.formVariants?.parent2
												?.details,
									deceased: deceased
										? deceased
										: questionnaireSettings?.step6_AddParent2GrandParents?.formVariants?.parent2
			...
			await dispatch(
				createRelativeAsync({
					...
					questionnaireCase: QuestionnaireCases.AddParent2GrandParent1,
					setting: {
						...questionnaireSettings,
						step6_AddParent2GrandParents: {
							isStep6Completed: false,
			...
			{props => (
				<>
					<Box alignItems="center" justifyContent="center" mt={15} mb={5} width={'100%'}>
						<Box width={'90%'}>
							<Text my={1}>
								Would you like to add details about your Maternal{' '}
								{parent1 ? 'Grandfather' : 'Grandmother'}?
							</Text>
							<Radio.Group
								name="details"
								colorScheme="amber"
...
export default AddGrandParents2Form;
